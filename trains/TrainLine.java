...
public class TrainLine {
    ...
    public void delete(String name) {
        // if head is null (if name entered doesnt exist), then print the station desired to be deleted does not exist
        if (this.head == null){
            System.out.println("The line you want to delete does not exist");
        // the other case is the x does exist, if x does exist then delete it
        } else {
    ...
    public void deleteDelete(String name){
        ...
        // x = name, the other case is the x does exist, if x does exist then delete it
        } else {
            if (this.head.getName().equals(name)) {
                // Remember what's after the head; we'll need it.
                ...
                // new variable named namePlusNext that is equal to the prior node's next
                TrainStation namePlusNext = prior.getNext();

                // replace prior with namePlusNext
                if (namePlusNext.hasNext()) {
                    // Pointer prior has a non null next, so it is not the last station.
                    // Deletion can proceed.
                    // replace prior with namePlusNext and get its next node (the second one)
                    prior.setNext(namePlusNext.getNext().getNext());
                } else if (!namePlusNext.hasNext()){ // if it does not have a next node
                    ...
                    prior.setNext(namePlusNext.getNext());
