...
public class SortingPractice {
    ...
    static int[] iterativeMergeSort(int[] array) {
        // create a temporary one element array to move our array[] 
        int[] tempArray = new int[0]; // 0 for 1 element index
        // for loop to iterate through the given array
        for (int i=1; i<array.length/2; i=i*2){ // partition into single elements from given array (use power of 2)
            for (int k=0; k<array.length; k=k+2*k){ // loop thru the single elements and use given indice
                tempArray = merge(slice(array, i, i+i), slice(array, i+i, i+2*i)); // merge the sliced single elements into pairs
                // sort pairs in tempArray into given array and reposition
                for(int x=0; x<tempArray.length; x++){
                    array[i+x] = tempArray[x]; // take 2 single elements (i and x) and pair into element (x)
                }
            }
        }
        return array; 
