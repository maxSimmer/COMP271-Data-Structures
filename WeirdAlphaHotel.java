...
/**
 * Finish remove method, whichRoom method, and isGuestHere method.
 */
...
public class WeirdAlphaHotel {
    ...
    public String remove(int room) {
        ...
        // if hotel has guests
        if (getUsage() > 0){
            // departing guest is equal to the guests name of the specified room number
            departingGuest = getGuestName(room);
            // set the specufied room back to empty
    ...
    public int whichRoom(String guestNane) {
        // assume no guest is found at first, so return -1
        int room = -1;
        // for loop to loop through EACH of the number of rooms
        for (int i=0; i<numberOfRooms; i++){
            // if the room is null (empty) go to the next
            if (this.hotel[i] == null){
                continue;
            // else if the given name is found, set the room to that room number where it is was found
            } else if (this.hotel[i].equals(guestNane)){
                // set the room to the room number correlating to the found name
                room = i;
            }
        }
        // Write this method to fulfill the functionality described in its Javadoc
    ...
    public boolean isGuestHere(String guestname) {
        // for loop to loop through EACH room
        for (int i=0; i<numberOfRooms; i++){
            // if the room is null (empty) go to the next
            if (this.hotel[i] == null){
                continue;
            // else if the room is equal to the given parameter, return true
            } else if (this.hotel[i].equals(guestname)){
                // if equal to given name return true
                return true;
            }
        }
